...
onMounted(() => {
  ...
  // 监听复合输入事件，解决中文输入法问题
  // 组合输入事件处理
const compositionHandler = (isComposing: boolean) => {
  if (isComposing) {
    // 标记当前处于组合输入状态
    (terminal as any).compositionHelper.isComposing = true;
  } else {
    // 结束组合输入状态
    (terminal as any).compositionHelper.isComposing = false;
  }
};

if (terminalContainer.value) {
  terminalContainer.value.addEventListener('beforeinput', (e: InputEvent) => {
    if ((terminal as any).compositionHelper?.isComposing) {
      // 如果当前处于组合输入状态，忽略输入事件
      e.preventDefault();
      return;
    }

    const input = e.data; // 获取输入内容
    if (input) {
      socket.send(input); // 将输入内容发送到后端
